...
def fermatFac(n):
	a = int(math.ceil(math.sqrt(n))) #our a is the ceiling of the sqrt, note that all our calculations are done with whole numbers hence the int conversion.
	b2 = a*a-n #our b2 is the square of a - the original value
	b = int(math.sqrt(b2)) #our b is the square root of b2, this is part of our factor if our b2 is infact a perfect square
...
def baby_giant(p,g,beta):
	...
	babystep.append([1,0]) #the first number for babystep is guranteed to be 1 and the index is 0. We store everything as tuples because we need indexes
	...
	while i <= m:
		...
		if i != 0: # this weeds out the first case, because we already did it individually near the top
			babystep.append([pow(g,i,p),i]) # for the rest of it raise g to each i and store that value and the index
		i+=1
	#print babystep
	print "\n"
...
def inverse(x,p): 
	ret = egcd(x,p)[1] #egcd returns multiple things in this order : gcd, inverse of x, inverse of y. We only want the inverse of x
	if ret < 0:
...
def main():		
	while True:
		...
		if choice == 0:
			...
			y = int(raw_input("Please enter the number of iterations to do this(The higher this number, the more accurate your result is): "))
			miller_rabin(x,y)
		elif choice == 5:
			x = int(raw_input("Please enter the number to be checked: "))
